...
function App() {
  const rawNights = useGoogleSheet();
  const nights = processNights(rawNights);

  const [currentIndex, setCurrentIndex] = useState(0);

  if (!nights[currentIndex]) return <div>Loading...</div>;

  return (
    <div>
      <Leaderboard night={nights[currentIndex]} />
      ...
      <div style={{ marginTop: "1rem" }}>
        ...
        {currentIndex < nights.length - 1 && (
          <button onClick={() => setCurrentIndex(currentIndex + 1)} style={{ marginLeft: "1rem" }}>
            Next
          </button>
        )}
      </div>
